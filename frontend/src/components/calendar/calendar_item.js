...
class CalendarItem extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.completedMeals) {
      if (
        ...
        Object.keys(this.props.completedMeals).length
      ) {
        if (this.props.completedMeals[this.props.day]) {
          bool = this.props.completedMeals[this.props.day].includes(
            this.props.id
          )
            ? true
            : false;
  ...
  componentDidMount() {
    let bool;
    if (
      this.props.completedMeals &&
      Object.keys(this.props.completedMeals).length > 0 &&
      this.props.completedMeals[this.props.day]
    ) {
      bool = this.props.completedMeals[this.props.day].includes(this.props.id)
        ? true
        : false;
      this.setState({ completed: bool });
    }
  ...
  handleUndo(e) {
    ...
    let completedMeal = `{"weekday": "${this.props.day}", "mealId": ${
      this.props.id
    }, "completed": ${false}}`;
    let parsedMeal = JSON.parse(completedMeal);
    const weekday = parsedMeal["weekday"];

    let modifiedCompletedMeals = JSON.parse(
      JSON.stringify(this.props.completedMeals)
    );
    ...
    if (parsedMeal["completed"]) {
      ...
      modifiedCompletedMeals[weekday] = modifiedCompletedMeals[weekday].filter(
        (id) => id !== parsedMeal["mealId"]
      );
    }

    this.setState({ completed: false });
    UserAPIUtil.updateCompletedMeals(
      modifiedCompletedMeals,
      this.props.completedMealCount - 1
    );
    this.props.receiveCompletedMeal(
      this.props.completedMealCount - 1,
      completedMeal
    );
  }
  ...
  displayCompleteButton() {
    if (this.props.futureDays.includes(this.props.day)) {
      ...
      return (
        <button id="complete-btn" onClick={this.handleUndo}>
          Undo
        </button>
      );
    } else {
      return (
        <button id="complete-btn" onClick={this.handleComplete}>
          Complete?
        </button>
      );
    }
